...
void mainEventHandler::onNotify(const char* topic, const char* message) {
  ...
  if (strcmp(topic, "Start_Client") == 0 && !client.is_client_setup) {
  ...
  if(strcmp(topic, "system_update/data") == 0 && client.is_client_setup){
    //std::cout << "Sending data to MQTT: " << message << std::endl;
    client.publishToTopic(ConfigManager::getInstance().mqttConfig.data_topic.c_str(), message);
  }
}
...
void setup(){
  ...
  sys.registerObserver(&eventHandler); // for mqtt pubs
